...
namespace Community.Areas.Identity.Pages.Account
{
    ...
    public class LoginModel : PageModel
    {
        ...
        public LoginModel(SignInManager<CommunityUser> signInManager,
        ...
        public async Task<IActionResult> OnPostAsync(string returnUrl = null)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                if (result.Succeeded)
                {
                    ...
                    if (user.CurrentLogin != DateTime.MinValue)
                    {
                        CommunityUser.LastLogin = user.CurrentLogin;
                    }
                    else
                    {
                        CommunityUser.LastLogin = DateTime.Now;
                    }

                    user.CurrentLogin = DateTime.Now;

                    TimeSpan timeSpan = user.CurrentLogin - CommunityUser.LastLogin;
                    user.Days += timeSpan.Days;
                    if (user.Days > 30)
                    {
                        user.Days = 0;
                        user.NoOfLogins = 0;
